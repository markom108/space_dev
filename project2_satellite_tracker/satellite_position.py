...
SITE= "https://celestrak.org/NORAD/elements/stations.txt" #AKTUALIZOWANE CO KILKA DNI/H, strona która zawiera TLE twojego oobiektu (kształt orbity, prędkość kątową, pozycję w danym momencie, zmiany w czasie)
...
while True:
    ...
    if count%FUTURE_TRAJ==0: # czy teraz wyliczamy kolejną przyszłą trajektorię
        ...
        satellites = sf.load.tle_file( SITE ) #DO obliczenia przyszłości, lista obiektów EarthSatellite ze sprasowanymi danymi TLE (połaczeone 3 linijki stąd .name .line1 .line2) 
        for i in satellites:
            if i.name==NAME:
                satellite=i
        if satellite==None:
