...
def RandomForest():
    global TEST_SIZE,SEED,DATA_PATH, TEST_GROUP_SIZE, RESOLUTION
    ...
    for count in range(TEST_GROUP_SIZE):
        ...
        mask=(mask>0).astype(np.float32) #zamień wartości 0-255 na bool i skonwertuj T/F na 0/1
    ...
    model=RandomForestClassifier(n_estimators=100, random_state=SEED, n_jobs=-1)#Tworzenie: liczba drzew decyzyjnych, ustalam "ziarno losowości"- to liczba, która ustala punkt startowy dla generatora liczb pseudolosowych w modelu. Dzięki temu wszystkie losowe decyzje (np. wybór próbek i cech w Random Forest) stają się powtarzalne – ten sam random_state daje zawsze te same wyniki treningu, ale nie zmienia jakości modelu| równoległy trening
...
def test_model(model):
    ...
    # Ścieżki
    path_red = os.path.join(DATA_PATH, "train_red", red_name)
    path_green = os.path.join(DATA_PATH, "train_green", green_name)
    path_blue = os.path.join(DATA_PATH, "train_blue", blue_name)
    path_gt = os.path.join(DATA_PATH, "train_gt", gt_name)

    # Wczytanie warstw
    red_layer = cv2.imread(path_red, cv2.IMREAD_UNCHANGED).astype(np.float32)
    green_layer = cv2.imread(path_green, cv2.IMREAD_UNCHANGED).astype(np.float32)
    blue_layer = cv2.imread(path_blue, cv2.IMREAD_UNCHANGED).astype(np.float32)

    # Normalizacja do 0-1
    red_layer /= red_layer.max()
    green_layer /= green_layer.max()
    blue_layer /= blue_layer.max()

    # Stworzenie obrazu RGB
    rgb = np.dstack((red_layer, green_layer, blue_layer))

    # Wczytanie maski GT
    mask_gt = cv2.imread(path_gt, cv2.IMREAD_UNCHANGED)
    mask_gt_binary = (mask_gt > 0).astype(np.float32)

    # Przygotowanie danych do predykcji
    features = rgb.reshape(-1, 3)
    predicted_mask = model.predict(features)
    predicted_mask = predicted_mask.reshape(mask_gt.shape)
    ...
    plt.imshow(mask_gt_binary, cmap='gray')
    plt.axis('off')
